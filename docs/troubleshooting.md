...
Reading controller input is gated by **Input Monitoring**. If `cc-controller
discover` keeps printing `listening, 0 events` while you press buttons:
...
2. Add/enable the **terminal app that hosts the daemon** (Ghostty / iTerm /
   ...
   than adding the bare `cc-controller` binary. (Click **+**, **⌘⇧G** to type a path.)
...
daemon, a backgrounded `… start` run — see `… status` / `… stop`) before starting another.
...
- **Cross-platform:** the SDL layer is portable; only `dictation_command` and
